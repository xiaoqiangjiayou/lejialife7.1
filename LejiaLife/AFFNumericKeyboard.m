...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        self.bounds = CGRectMake(0, 0,KSCREEN_WIDTH , 216);
        //
//        arrLetter = [NSArray arrayWithObjects:@"qqq",@"DEF",@"GHI",@"JKL",@"MNO",@"RST",@"UVW",@"XYZW", nil];
        ...
        //
        UIView *line1 = [[UIView alloc] initWithFrame:CGRectMake(KSCREEN_WIDTH/3, 0, kLineWidth, 216)];
        line1.backgroundColor = color;
        [self addSubview:line1];
        
        UIView *line2 = [[UIView alloc] initWithFrame:CGRectMake(KSCREEN_WIDTH/3*2, 0, kLineWidth, 216)];
        line2.backgroundColor = color;
        [self addSubview:line2];
        
        for (int i=0; i<3; i++)
        {
            UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, 54*(i+1), KSCREEN_WIDTH, kLineWidth)];
...
-(UIButton *)creatButtonWithX:(NSInteger) x Y:(NSInteger) y
{
    ...
    switch (y)
    {
        case 0:
            frameX = 0.0;
            frameW = KSCREEN_WIDTH/3;
            break;
        case 1:
            frameX = KSCREEN_WIDTH/3;
            frameW = KSCREEN_WIDTH/3;
            break;
        case 2:
            frameX = KSCREEN_WIDTH/3*2;
            frameW = KSCREEN_WIDTH/3;
            break;
    ...
    if ( num == 12)
    {
        ...
        colorNormal = colorTemp;
        colorHightlighted =colorHightlighted;
    }
    ...
    {
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 8, frameW, 28)];
        label.text = @".";
        label.textColor = [UIColor blackColor];
        label.textAlignment = NSTextAlignmentCenter;
        label.font = kNumFont;
    ...
    {
        UIImageView *arrow = [[UIImageView alloc] initWithFrame:CGRectMake(45, 19, 22, 17)];
        arrow.image = [UIImage imageNamed:@"delete@2x.png"];
...
//        [self.delegate changeKeyboardType];
        [self.delegate pointNumberKeyboardInput];
